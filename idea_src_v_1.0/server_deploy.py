...
def response(message,list_new):
	status = 1
	
	if list_new == None or len(list_new) == 0 :
		status = 0
	new_out = json.dumps(list_new)
	ret_val={'message':message,'status':status,'data':new_out}
