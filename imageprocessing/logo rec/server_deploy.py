...
def hello():
	try:
		...
		# check if the post request has the file part
			...
			img = cv2.imdecode(data, color_image_flag) #img is the image
			v = dummy5.image_calc(img)
		
			list1 = v.values()
			if list1[0] == []:
				ret_val={'x-image-profile-id':header_req,'message':'images not found','status':0,'data':list1 }
				return 	jsonify(**ret_val)
			else:	
				ret_val={'x-image-profile-id':header_req,'message':'images found','status':1,'data':list1 }
				return 	jsonify(**ret_val)			
		
	except:
		ret_val={'x-image-profile-id':header_req,'message':'server error.(Load Data first then do the search)','status':0,'data':'No Data' }
		return 	jsonify(**ret_val)
...
def check_status(name):
	search_in2 = pd.read_csv(data["ImageCsv"],sep=',',header=None)
